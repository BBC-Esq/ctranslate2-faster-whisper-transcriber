...
class ModelLoaderThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.error_occurred.emit(f"Error loading model: {e}")
...
class VoiceRecorder(QObject):
    ...
    def on_model_loaded(self, model, model_name):
        ...
        self.update_status_signal.emit(f"Model updated to {model_name} on {self.model_loader_thread.device_type}")
    ...
    def transcribe_audio(self, audio_file):
        ...
        mid = id(m)
        self.model_mutex.unlock()
        self.transcription_thread = TranscriptionThread(m, mid, audio_file)
